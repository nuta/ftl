use ftl_types::{error::FtlError, syscall::{SyscallNumber, VsyscallPage}};
use spin::Mutex;

static VSYSCALL_PAGE: Mutex<Option<&'static VsyscallPage>> = Mutex::new(None);

pub fn syscall(n: SyscallNumber, a0: isize, a1: isize, a2: isize, a3: isize, a4: isize) -> Result<isize, FtlError> {
...
pub fn print(s: &[u8]) -> Result<(), FtlError> {
    syscall(SyscallNumber::Print, s.as_ptr() as isize, s.len() as isize, 0, 0, 0)?;
    Ok(())
}
