...
impl ArpTable {
    ...
    pub fn add(&mut self, addr: Ipv4Addr, mac: MacAddr) -> Result<(), OutOfMemoryError> {
        self.entries
            .reserve_and_insert(addr, ArpEntry::Resolved(mac))?;
        Ok(())
    }
...
pub(crate) fn handle_rx<I: Io>(tcpip: &mut TcpIp<I>, pkt: &mut Packet) -> Result<(), RxError> {
    ...
    match arp.opcode.into() {
        OPCODE_REQUEST => {
            if let Some(iface) = tcpip.interfaces_mut().get_mut_by_ipv4_addr(target_addr) {
                // Register the sender's MAC address so that we can reply to it.
                if let Err(err) = iface.arp_table_mut().add(sender_addr, arp.sender_hw_addr) {
                    debug!("failed to add ARP entry for {}: {:x?}", sender_addr, err);
                }
