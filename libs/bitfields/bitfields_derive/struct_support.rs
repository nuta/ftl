...
fn visit_fields(input: &FieldsNamed) -> Vec<Field> {
    ...
    for field in &input.named {
        let field_ident = field.ident.clone().expect("failed to get the field ident");
...
pub fn bitfields_struct(
    ...
    } in &fields
    {
        ...
        // Offset: foo_offset()
        let offset = Ident::new(&format!("{}_offset", ident), Span::call_site());
        if prev_fields.is_empty() {
            ...
            methods.push(quote! {
                #[inline]
                #vis const fn #offset() -> usize {
                    #(<#prev_types as ::ftl_bitfields::BitField>::BITS)+*
                }
            });
        }
        ...
        if !hidden {
            ...
            if readable {
                ...
                methods.push(quote! {
                    #[inline(always)]
                    pub fn #getter(&self) -> <#ty as ::ftl_bitfields::BitField>::AccessorValueType {
                        let mask = ((1 << Self::#width()) - 1) << Self::#offset();
                        let value = (self.raw & mask) >> Self::#offset();
                        <#ty as ::ftl_bitfields::BitField>::from_u64(value as u64)
                    }
                });
            }
            ...
            if writable {
                ...
                methods.push(quote! {
                #[inline(always)]
                pub fn #setter(&mut self, value: <#ty as ::ftl_bitfields::BitField>::AccessorValueType) {
                    let value = <#ty as ::ftl_bitfields::BitField>::into_u64(value) as #struct_width;
