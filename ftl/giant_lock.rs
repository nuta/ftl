...
struct LockTracker {
    lock: AtomicBool,
    locked_at: Cell<Option<Backtrace>>,
}
...
impl LockTracker {
    ...
    fn acquire(&self, locked_at: Backtrace) {
        if self
            .lock
...
/// multiple CPUs).
///
/// Each `GiantLock` object can be borrowed dynamically and independently,
/// but unlike mutexes, they internally share the same lock, aka "Big Kernel Lock".
///
/// The giant lock will automatically be held in `arch` module when it enters
...
impl<T> GiantLock<T> {
    ...
    pub fn borrow_mut(&self) -> GiantLockGuard<'_, T> {
        ...
        self.tracker.acquire(Backtrace::capture());
